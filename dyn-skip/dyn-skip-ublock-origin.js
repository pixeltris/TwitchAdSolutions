...
(function() {
        ...
        constructor(twitchBlobUrl) {
            ...
            var newBlobStr = `
                ...
                self.addEventListener('message', function(e) {
                    if (e.data.key == 'set_gql_device_id' && gql_device_id != e.data.value) {
            ...
            this.onmessage = function(e) {
                if (e.data.key == 'UboShowAdBanner') {
                    ...
                    if (resetButton != null && videoPlayerElement != null) {
    ...
    async function stripAds(url, textStr, realFetch) {
        ...
        if (haveAdTags && !streamInfo.AttemptedSkip) {
            streamInfo.AttemptedSkip = true;
            var matches = textStr.match(/#EXT-X-DATERANGE:(ID="stitched-ad-[^\n]+)\n/);
            if (matches.length > 1) {
                        ...
                        while(true) {
                            ...
                            if (encodingsM3u8Response.status === 200) {
                                ...
                                if (!streamM3u8.includes(AD_SIGNIFIER) || ++attempts >= maxAttempts) {
                                    ...
                                    streamInfo.BackupFailed = false;
                                    streamInfo.AttemptedSkip = false;
        ...
        window.fetch = function(url, init, ...args) {
            if (typeof url === 'string') {
                if (url.includes('gql')) {
                    ...
                    if (typeof deviceId === 'string' && twitchMainWorker) {
                        twitchMainWorker.postMessage({key:'set_gql_device_id',value:deviceId});
                    }
                }
